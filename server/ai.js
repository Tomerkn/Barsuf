...
הצג 3-5 נקודות קצרות וברורות בלבד שיוצגו ללקוח. ענה בעברית.
הוראת חובה: אין להשתמש באייקונים או אמוג'י בשום פנים ואופן (למעט ✅ או ❌ במידת הצורך). שמור על ניסוח מקצועי, נקי ואנושי.
...
הוראת חובה: אסור בשום אופן להשתמש באייקונים או באמוג'י (למעט התווים ✅ או ❌ עבור ההמלצה בלבד). עצב את הטקסט בצורה חלקה, עניינית, ורשמית בלבד!

על הניתוח להיות במבנה הבא בדיוק:
### ניתוח: [שם המכרז]
**תמצית:** [משפט אחד קצר המסכם את המכרז]
**סיווג:** [סיווג קבלני]. **ערבות:** [סכום] ₪. **ביצוע:** [זמן] חודשים.
**המלצה:** ✅ להגיש. (או ❌ לא להגיש, בהתאם לניתוח)

התייחס בנוסף ל: תנאי סף, לוחות זמנים, קנסות, ערבויות, ודרישות ביטוח בפיסקה קצרה וללא אייקונים.
חובה לכלול פרק בשם "שלבי הביצוע להנגשה ללקוח" המתרגם את המכרז לשלבי עבודה פשוטים וברורים.
...
דרישות:
1. הצעת המחיר הכתובה צריכה לכלול: מכתב פתיחה רשמי ומקצועי ללקוח, ופרק מיוחד ומסודר בשם "שלבי ביצוע להצגה ללקוח" המפרט בצורה מונגשת, פשוטה וברורה לקורא הלא-מקצועי את שלבי העבודה השונים.
2. חל איסור מוחלט לפרט או לכתוב את סעיפי כתב הכמויות (הסעיפים, כמויות, מחירי יחידה, וסה"כ) כרשימה או כטקסט בגוף ההצעה הכתובה, מכיוון שהם יוצגו אוטומטית בטבלה נפרדת ומקצועית במערכת.
3. חובה לכלול בלוק JSON בלבד בסוף התשובה המכיל את כתב הכמויות (BoQ) המפורט. על מחירי היחידה להיות מבוססים על היסטוריית המחירים שלנו. מבנה הבלוק יהיה בפורמט הבא בדיוק:
\`\`\`json
[{"id":1,"section":"שם סעיף","item":"תיאור פריט","quantity":100,"unit":"מ\"ר","unitPrice":150}]
\`\`\`
4. חל איסור מוחלט להשתמש במילים "JSON", "בלוק קוד" או להתייחס לכתב הכמויות הטכנולוגי בגוף ההצעה הכתובה.
5. כאשר נדרש למלא את פרטי המציע, החברה המציעה, מורשה החתימה, תפקיד או חתימה וחותמת - חובה למלא תמיד את הפרטים הבאים בדיוק:
   - שם המציע / החברה המציעה: ברסוף בע״מ
   - שם מורשה חתימה: שמעון אזולאי
   - תפקיד: מנכ״ל
   - חתימה וחותמת החברה: שמעון אזולאי, מנכ״ל - ברסוף בע״מ
6. חל איסור מוחלט להשתמש באייקונים, אימוג'ים (emojis) או סמלים גרפיים כלשהם בטקסט.
7. סגנון הכתיבה צריך להיות אנושי, טבעי ומקצועי, ללא ניסוחים רובוטיים או תבניתיים מדי.
8. חובה לסיים את התשובה כולה עם תגית הוודאות: [CONFIDENCE]XX[/CONFIDENCE]`;
